				...
				<div className='history-container'>
					{moodEntries.length === 0 ? (
						<div className='empty-state'>
							...
							<IonButton className='add-mood-button' onClick={() => history.push('/mood-selector')}>
						...
						Object.keys(groupedEntries).map(dateKey => (
							<div key={dateKey} className='history-group'>
								...
								{groupedEntries[dateKey].map(entry => (
									<div key={entry.id} className='history-item' onClick={() => handleEntryClick(entry.id)}>
										<div className='history-item-emoji'>{entry.emoji.emoji}</div>
										<div className='history-item-content'>
											<div className='history-item-name'>{entry.emoji.name}</div>
											<div className='history-item-notes'>{entry.notes || 'Sin notas'}</div>
										</div>
